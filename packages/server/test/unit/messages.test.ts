...
describe('parseInboundFrame', () => {
  it('acquire_lock', () => {
    expect(parseInboundFrame(JSON.stringify({ type: 'acquire_lock', region: 'A1:A1' }))).toEqual({
      type: 'acquire_lock',
      region: 'A1:A1',
    })
  })
  ...
  it('release_lock', () => {
    expect(parseInboundFrame(JSON.stringify({ type: 'release_lock', region: 'A1:A1' }))).toEqual({
      type: 'release_lock',
      region: 'A1:A1',
    })
  })
