...
test('two contexts: only one wins the same-cell lock', async ({ browser }) => {
  ...
  await pageA.waitForFunction(
    () =>
      typeof (window as { ensembleAcquireLock_admin?: unknown }).ensembleAcquireLock_admin ===
      'function',
    { timeout: 30_000 },
  )
  ...
  await pageB.waitForFunction(
    () =>
      typeof (window as { ensembleAcquireLock_viewer?: unknown }).ensembleAcquireLock_viewer ===
      'function',
    { timeout: 30_000 },
  )
  ...
  const [a, b] = await Promise.all([
    pageA.evaluate(() =>
      (
        window as unknown as {
          ensembleAcquireLock_admin: (r: string) => Promise<{ acquired: boolean; ownerId: string }>
        }
    ...
    pageB.evaluate(() =>
      (
        window as unknown as {
          ensembleAcquireLock_viewer: (r: string) => Promise<{ acquired: boolean; ownerId: string }>
        }
